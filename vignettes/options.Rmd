...
description: >
  Learn how to customize the behaviour of acronymsdown.
...
**Acronymsdown** supports various options to change its behaviour.
This vignette lists them, what they do, and their allowed values.

Options should be set in the YAML metadata, inside the `acronyms`
field.
...
automatically and insert it into your document.
This option controls the title (header) that precedes this list.
...
By default, **acronymsdown** generates a List Of Acronyms
...
might not be wanted. In this case, this option can be used to
disable it.
...
will be replaced by **acronymsdown** with the List Of Acronyms.
...
---
acronyms:
  insert_beginning: false
---
```

To generate and insert the List Of Acronyms where you want:
```md
...
- `usage`: Acronyms are sorted in the order in which they
  ...
  **acronymsdown** will raise an error, since it cannot sort
...
This option controls what to dot when **acronymsdown** finds
...
By default, **acronymsdown** prints a warning to the standard
...
- `warn` (default)
- `replace`: No warning is issued, and the old acronym is
...
In the following examples, we consider the following
acronyms definitions in the YAML metadata:
```yaml
---
acronyms:
  keys:
...
By default, **acronymsdowns** expect to find acronyms definitions
inside the YAML metadata~; however, you can use `fromfile`
to specify external YAML files from which acronyms should be
loaded.

Its usage is described in details in `vignette("advanced_usage")`.

## `style`

**Acronymsdown** supports several styles to replace acronyms,
similarly to how the LaTeX package [glossaries] work for
abbreviations.
...
of them, is displayed in `vignette("styles")`.
